...
const Chatbot = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const chatResponse = messageResponse.response; // The main response text
      const metadata = messageResponse.metadata; // Metadata object

      let displayMessage = chatResponse;

      // Check if metadata exists and is an object
      if (metadata && typeof metadata === 'object') {
        Object.keys(metadata).forEach(key => {
          const fileInfo = metadata[key];
          if (fileInfo && fileInfo.file_name && fileInfo.page_label) {
            displayMessage += `\n\n${fileInfo.file_name} (Page ${fileInfo.page_label})`;
          }
        });
      }

      setMessages([...messages, { text: usermessage, sender: 'user' }, { text: displayMessage, sender: 'bot' }]);
      setHistory([...history, usermessage, displayMessage]);
    } catch (error) {
